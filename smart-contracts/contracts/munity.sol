...
contract  Munity is ERC1155, ERC2981, Ownable, ReentrancyGuard {
    ...
    /// @dev Maximum allowed platform fee in BASE units (10%).
    ...
    function buy(uint256 _id, uint256 _amount) external payable nonReentrant {
       (uint256 price, uint256 supply, uint256 discount, address creator) = getCommunityDetails(_id);
        ...
        uint256 amountToBeReturn;
        uint256 payToCreator;
        uint256 payToOwner;

        if(sender != creator) {
            if(isWhiteListed(_id, sender)) price = discount == BASE ? 0 : price - ((price * discount) / BASE);
            uint256 totalAmt = price * _amount;
            if(msg.value < totalAmt) revert MoneyNotEnough(totalAmt);
            amountToBeReturn = msg.value - totalAmt;
            payToOwner = (totalAmt * communityFee) / BASE;
            payToCreator = totalAmt - payToOwner;
        }
        ...
        if(payToOwner > 0) {
            (bool success, ) = payable(owner()).call{value: payToOwner}("");
            if(!success) revert TransferFailed();
        }
    ...
    function changeCommunityFee(uint256 _newFees) external onlyOwner {
        if(_newFees > MAX_FEE) revert FeeTooHigh(MAX_FEE);
